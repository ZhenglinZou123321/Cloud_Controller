...
class JunctionController():
    def __init__(self, junction_id_,traffic_light_to_lanes_,N,dt,L_safe,redis_base,task1_ip,task1_port):
    ...
    def get_last_quarter_every_lane(self):
        ...
        for laneID in lane_ids:
            ...
            vehicles_in_last_quarter = [
                ...
                if Global_Vars.VehicleLib[vehID].laneposition > 0.75 * lane_length
            ]
            self.last_quarter_vehicles[laneID] = vehicles_in_last_quarter #idx=0的车是最靠近lane终点的，以此类推

    def update_cav_speeds(self,step):
        print('updating_cav_speed')
        ...
        for lane_id in self.traffic_light_to_lanes[self.junction_id]:
            ...
            if len(initial_state_CAV) != 0:
                ...
                self.redis_base.hset(key,'vehicles_list_this_lane',pickle.dumps(vehicles_list_this_lane))
                ...
                self.redis_base.hset(key,'lane_id',pickle.dumps(lane_id))

                self.junction_socket.send(pickle.dumps(key)+end)
                print(key)
                msg = b''
                while True:
                    recv_data = self.junction_socket.recv(1024)  
                    # 接收u
                    msg = msg + recv_data
                    if end in msg[-25:]:
                        break
                self.junction_socket.send(reply)
                datas = pickle.loads(msg)
                u = datas
